# -*- coding: utf-8 -*-
"""
Created on Thu Dec  3 11:15:03 2020
...
def k_means_clustering(data, centroids, k):
    ...
    while not centroid_current.equals(centroid_last):
        ...
        for idx, position in centroid_current.iterrows():
            # the idx will not be used. We just separate the index with the actual data "position" in the dataset. You will use "cluster_count" as the column index in "clusters".
            # your code is here. Save the Euclidean distances into 'clusters'
            newData=(data[centroid_current.columns] -np.array(position))
            clusters[cluster_count] = (newData).pow(2).sum(1)**(1/2)
            # your code ends
            cluster_count += 1  
        ...
        for j in range(len(data)):
            ...
            if (data['Cluster'][j] == 0):
                l1.append([x,y])
            elif(data['Cluster'][j] == 1):
                l2.append([x,y])
            elif(data['Cluster'][j] == 2):
                l3.append([x,y])
       #add calculated values to the points
        points.append(l1)
        points.append(l2)
        points.append(l3)
        # your code ends
